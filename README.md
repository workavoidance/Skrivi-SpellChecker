...
This repository contains research and working evaluation tools. **It does not yet contain the earlier runnable Nuspell/NorBERT sandbox.** Its location is unresolved; a new baseline has not been measured. See [STATUS.md](STATUS.md) for the verified inventory and next work.
...
- `sandbox/`: reserved for the recovered runnable application; currently documentation only.
...
Create the local input/output folders as needed. There is no bundled private dataset or launch command for the missing sandbox.

## Local data and privacy

Keep private writing, dataset downloads and results in the ignored `data/` and `results/` folders. Never commit pupil text or derived records without explicit redistribution permission. Model weights belong in a persistent user cache outside the app, intended location `%LOCALAPPDATA%\Skrivi\models`; no runtime cache implementation is included yet.
