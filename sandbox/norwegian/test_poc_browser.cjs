...
(async()=>{
 ...
 try{
  ...
  await page.screenshot({path:path.join(__dirname,'poc-desktop.png'),fullPage:true});
  await page.setViewportSize({width:390,height:844});
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+1));
  await page.screenshot({path:path.join(__dirname,'poc-mobile.png'),fullPage:true});
  ...
  fs.writeFileSync(path.join(__dirname,'poc-verification.json'),JSON.stringify({
